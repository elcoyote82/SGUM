...
<div id="tituloTabla" >
	<?php echo __("LISTADO DE USUARIOS DEL PERMISO '".$nombrePermiso."'"); ?>
</div>
<?php if($pager->getnbResults() == 0): ?>
	<table class='centrarTablas'>
	  <tr>
	    <td>
	      <?php echo __("No se encontró ningún usuario del permiso ".$nombrePermiso."."); ?>
	    </td>
	  </tr>
	  <tr>
	...
	<div align='center'>
	      <ul class='paginator'>
	        <?php if ($pager->haveToPaginate()): ?>
	          <li class='previous'><?php echo link_to('&laquo;', 'administracion/verUsuariosPermiso?page='.$pager->getFirstPage()) ?></li>
	          <li class='previous'><?php echo link_to('&lt;', 'administracion/verUsuariosPermiso?page='.$pager->getPreviousPage()) ?></li>
	          <?php $links = $pager->getLinks(); 
	          foreach ($links as $page): ?>
	            <?php if($page == $pager->getPage()): ?>
	              <li class='current'><?php echo $page; ?></li>
	            <?php else: ?>
	              <li class='page'><?php echo link_to($page, '/administracion/verUsuariosPermiso?page='.$page) ?></li>
	            <?php endif; ?>
	            <?php if ($page != $pager->getCurrentMaxLink()): ?><li class='empty'>   <li><?php endif ?>
	          <?php endforeach ?>
	          <li class='next'><?php echo link_to('&gt;', 'administracion/verUsuariosPermiso?page='.$pager->getNextPage()) ?></li>
	          <li class='next'><?php echo link_to('&raquo;', 'administracion/verUsuariosPermiso?page='.$pager->getLastPage()) ?>
	        <?php endif ?>
	      </ul>
	</div>
	<table class='centrarTablas'>
